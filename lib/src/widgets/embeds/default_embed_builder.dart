...
Widget defaultEmbedBuilder(
  ...
  switch (node.value.type) {
    ...
    case BlockEmbed.linkPreviewType:
      ...
      if (isMobile() && style != null) {
        ...
        if (_attrs.isNotEmpty) {
          ...
          linkPreview = Padding(
            padding: EdgeInsets.all(m),
            child: Container(width: w, child: Text('LinkPreview: $linkUrl')),
          );
        }
      } else {
        linkPreview = SizedBox(
          width: MediaQuery.of(context).size.width,
          child: Container(
            color: Colors.blue.shade300,
            child: Text('Default link preview: $linkUrl'),
          ),
        );
      }
    ...
    case InlineBlockEmbed.hyperlinkType:
      // var text = node.value.data;
      // var link = '';
      // try {
      //   const symbol = '](';
      //   final index = text.indexOf(symbol);
      //   link = text.substring(index + symbol.length, text.length - 1);
      ...
      var link = '';
      try {
        var data = json.decode(node.value.data);
        data.forEach((k, v) {
          text = k;
          link = v;
        });
